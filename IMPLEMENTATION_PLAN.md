...
- Para a Fase 6, `docker-compose.iam.yml` inicia MiniStack 1.5.15 com `AUTH=true` e reutiliza o provisionamento local das filas. `scripts/verify_local_sqs_policies.sh` aplica os mesmos templates IAM a três identidades descartáveis; envio do produtor, consumo/delete e publicação do app passaram, enquanto sete ações indevidas retornaram `AccessDenied`. O MiniStack não valida a assinatura SigV4, logo o ensaio comprova avaliação das políticas, mas não autenticidade do segredo apresentado. A auditoria classifica esse requisito como parcial e não exige conta AWS real.
...
- A Fase 11 foi revalidada com um snapshot Git temporário das alterações atuais clonado em checkout sem modificações. O Compose isolado construiu o app, iniciou com volume novo, respondeu readiness, aplicou migrations `1,2,3,4` e criou as três filas. Tokens reais permitiram abrir carteira (`201`) e negaram acesso do provider (`403`). `go test -race -count=1 ./...`, `go vet ./...`, a matriz completa de integração com `-race` e o ensaio de políticas no MiniStack passaram no clone. Stacks, redes e volume descartáveis foram removidos.
...
A auditoria linha a linha da matriz está em `REQUIREMENTS_AUDIT.md`. As fases funcionais locais estão concluídas. A suíte MiniStack demonstra a avaliação de políticas por identidade declarada; a autenticação criptográfica não foi demonstrada porque o emulador não verifica SigV4. O enunciado não exige conta AWS real. Reavaliar planos SQL com dados e carga do ambiente alvo antes de alegar desempenho em escala.
