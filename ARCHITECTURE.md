...
Um teste de falha isolado usa proxies locais que podem cortar conexões PostgreSQL existentes e responder indisponibilidade no SQS, sem alterar os containers compartilhados. Erros de conexão fechada e de transporte do `pgx`, além dos SQLSTATEs transitórios, tornam-se `503 TRANSIENT_FAILURE`; o cliente deve repetir a identidade original, inclusive quando não souber se houve commit. A perda do SQS não desfaz o commit financeiro: a outbox permanece durável e o publisher retoma o envio após recuperação. A classificação de conexão fechada segue a semântica de `pgconn.ErrConnClosed` documentada no pgx 5.10.0. Revisão de planos de consulta e a revisão final de entrega ainda estão pendentes.
